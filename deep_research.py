...
def generate_pdf(topic, report_text, overall_score, claim_results):
    ...
    if overall_score is not None:
        if overall_score >= 70:
            box_r, box_g, box_b = 0, 180, 160
            verdict = "High Confidence"
        elif overall_score >= 45:
            box_r, box_g, box_b = 220, 140, 0
            verdict = "Medium Confidence"
        else:
            box_r, box_g, box_b = 210, 50, 50
    ...
    for line in lines:
        ...
        if safe_line.startswith("## "):
            ...
            pdf.multi_cell(190, 6, "  -  " + safe_line[2:])
...
def run_research(topic, firecrawl_api_key, max_urls, max_depth):
    ...
    with st.spinner("Crawling the web for information..."):
        try:
            ...
            final_report = results['data']['finalAnalysis']
    ...
    # ── STEP 2: Citation Accuracy Check ──
    st.markdown("### 🔬 Step 2: Citation Accuracy Analysis")
    with st.spinner("Verifying claims against source material..."):
        overall_score, claim_results = run_citation_check(final_report, source_texts)
    ...
    with col_md:
        st.download_button(
            ...
            data=final_report + citation_summary,
    ...
    with col_pdf:
        with st.spinner("Generating PDF..."):
            pdf_bytes = generate_pdf(topic, final_report, overall_score, claim_results)
        st.download_button(
            label="📕 Download PDF",
            data=pdf_bytes,
...
if st.button("🚀 Start Deep Research", type="primary"):
    if not firecrawl_key:
        ...
        run_research(research_topic, firecrawl_key, max_urls, max_depth)
